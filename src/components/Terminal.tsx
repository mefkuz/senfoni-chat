      ...
      try {
        ...
        for (const m of msgs) {
          ...
          try {
            ...
            const entry: LogEntry = { id: m.id, text: `${m.sender}: ${plain}`, type: 'msg', time: new Date(m.timestamp).toLocaleTimeString('en-US', { hour12: false, hour:'2-digit', minute:'2-digit', second:'2-digit' }), isOwn: m.sender === username, status: 'sent' };
            if (m.fileId) {
              ...
              if (!hasVideo) {
                return (
                  <div key={rs.peerId} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--bg-card)', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--border)', height: 'fit-content', alignSelf: 'flex-start' }}>
                    ...
                    <video autoPlay playsInline style={{ width: 0, height: 0, position: 'absolute', visibility: 'hidden' }} ref={v => { if (v && v.srcObject !== rs.stream) { v.srcObject = rs.stream; v.play().catch(()=>{}); } }} />
                  </div>
                );
              }
